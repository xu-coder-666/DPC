...
def load_h5(h5txt3D_path):         ###it's incomplete
    ...
    for filename in txt.split():
        ...
        data_ = np.array(h5['data'][:, :, :512, :])#.transpose(3, 2, 1, 0))   # (1547, 3, 1024, 6)
        label_ = np.array(h5['label'][:, :, :512, :])#.transpose(3, 2, 1, 0)) # (1547, 3, 1024, 3)
        ...
        #         data_[i, j, :, 3] = data_[i, j, :, 3] / 255
        #         data_[i, j, :, 4] = (data_[i, j, :, 4] - 0.5) / 255
        #         data_[i, j, :, 5] = (data_[i, j, :, 5] - 0.5) / 255
        #         data_[i, j, :, 0] = data_[i, j, :, 0] / 255
        #         data_[i, j, :, 1] = (data_[i, j, :, 1] - 0.5) / 255
        #         data_[i, j, :, 2] = (data_[i, j, :, 2] - 0.5) / 255
        
        # ***************************** 使用rgb颜色空间 *******************************
        # for i in range(data_.shape[0]):
        #     for j in range(data_.shape[1]):
        #         data_[i, j, :, 3:] = np.round(yuv2rgb(data_[i, j, :, 3:])) / 255
        #         label_[i, j, :, :] = np.round(yuv2rgb(label_[i, j, :, :])) / 255
        # ***************************** 使用rgb颜色空间 *******************************
...
if __name__ == '__main__':
    ...
    # traindata, label = load_h5('data_LT/h5_mix_new/trainFile1.txt')
    traindata, label = load_h5('data_LT/h5_mix_test_new/testFile1.txt')
    print(traindata[-1][0])
    print(label[-1][0])
    ...
    # pc = read_ply('data_LT/data_rec_add/longdress_vox10_1278_r01.ply')
    # print(pc[:10, :])